...
export const datosRouter = createTRPCRouter({
  ...
  // id_maquina = 0 → filtra solo por id_cliente (caso sin máquina vinculada)
  getSummaryDia: publicProcedure
    .input(
      z.object({
        id_cliente: z.number().int().positive(),
        id_maquina: z.number().int().nonnegative(),
        fecha: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
      }),
    )
    .query(async ({ ctx, input }) => {
      ...
      const rows = await ctx.db.$queryRaw<RawSummaryDia[]>`
        SELECT
          ...
          MIN(CASE WHEN b.estado <> 'OFF' THEN b.time END) AS inicio_func,
          MAX(CASE WHEN b.estado <> 'OFF' THEN b.time END) AS fin_func,
          ROUND(COALESCE(
            TIMESTAMPDIFF(SECOND,
              MIN(CASE WHEN b.estado <> 'OFF' THEN b.time END),
              MAX(CASE WHEN b.estado <> 'OFF' THEN b.time END)
      ...
      return rows.map((r): SummaryDia => {
        ...
        return {
          ...
          inicio_func:      r.inicio_func ? toDate(r.inicio_func) : null,
          fin_func:         r.fin_func    ? toDate(r.fin_func) : null,
